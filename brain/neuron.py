...
class brain:
    ...
    def __init__(self,layers=0,rows=0,cols=0):
        self.knowledges = []
        self.neurons = np.array([[[neuron(kk,jj,ii) for ii in range(cols)]
                         for jj in range(rows)]
                        for kk in range(layers)])
        brain.me=self

    @staticmethod
    def getNeuron(brainlayer,r,c):
        return brainlayer.neurons[r][c]

    def clear(self,la,top,left,w,h):
        for i in range(w):
            for j in range(h):
                n=self.getNeuron(la,j+top,i+left)
                n.value=0

                if ( hasattr(n, "dendritic")):
                    for s in n.dendritic.synapses:
                        n.dendritic.synapses.remove(s)
                        if(brain.synapses.count(s)>0):
                            brain.synapses.remove(s)

                if ( hasattr(self, "axon")):
                    for s in n.axon.synapses:
                        if(brain.synapses.count(s)>0):
                            brain.synapses.remove(s)
                        n.axon.synapses.remove(s)

    def reset(self,la,row,col,w,h):
        for i in xrange(w):
            for j in xrange(h):
                ##print la,w,h
                n = self.getNeuron(la, j + row, i + col)
                n.value = 0

    def input(self,image,pos,row,col):
        h=len(image)
        w=len(image[0])
        for i in range(h):
            for j in range(w):
                n=self.getNeuron(pos,i+row,j+col)
                n.value=image[i][j]


    def testconvolution(self, la, ra, ca, wa, ha, lb, rb, cb, cmat,cv):
        wc = len(cmat[0]);
        hc = len(cmat)
        #cv = np.array(cmat).sum()
        for j in range(int(wc/2), wa - int(wc/2)):
            for k in range(int(int(hc/2)), ha - int(int(hc/2))):
                ...
                ntmp.axon.criticalvalue = cv

    @staticmethod
    def convolution(la,ra,ca,wa,ha, lb,rb,cb, cmat):
        wc = len(cmat[0]);
        hc = len(cmat)
        cv = np.array(cmat).sum()
        synapses=[]
        ...
        return synapses
    @staticmethod
    def convolution(la,lb,cmat):
        ha=la.neurons.shape[0]
        wa=la.neurons.shape[1]
        brain.convolution(la, 0, 0, wa, ha, lb,cmat)


    @staticmethod
    def deconvolution(la,ra,ca,wa,ha, lb,rb,cb, cmat):
        wc = len(cmat[0]);
        hc = len(cmat)
        cv = np.array(cmat).sum()
        synapses=[]
        for j in range(int(wc/2),wa-int(wc/2)):
            for k in range(int(hc/2),ha-int(hc/2)):
                nla = brain.getNeuron(la,rb+k,cb+j)
                for a in range(wc):
                    for b in range(hc):
                        if cmat[b][a]:
                            nlb =brain.getNeuron(lb, ra+k + b - int(int(hc/2)), ca+j + a - int(wc/2))
                            s=nla.connectto(nlb,cmat[b][a])
                            synapses.append(s)
        return synapses
    ...
    def connectMat(self,n,la,ra,ca,mat):
        h=len(mat)
        w=len(mat[0])
        for r in range(h):
            for c in range(w):
                self.getNeuron(la,ra+r-h/2,ca+c-w/2).connectto(n,mat[r][c])
        n.axon.criticalvalue=h*w
    ...
    class BrainLayer:
        def __init__(self,rows,cols):
            self.neurons=np.array([[neuron(0,jj,ii) for ii in range(cols)]
                         for jj in range(rows)])
